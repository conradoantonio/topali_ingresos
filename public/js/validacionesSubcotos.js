...
var btn_enviar_producto = $("#guardar_subcoto");
btn_enviar_producto.on('click', function() {
    inputs = [];
    msgError = '';

    validarInput($('input#nombre_subcoto'), regExprTexto) == false ? inputs.push('Nombre subcoto') : ''
    ...
    validarSelect($('select#cotos_id')) == false ? inputs.push('Coto id') : ''
    ...
    if (inputs.length == 0) {
        $('#guardar_subcoto').hide();
        $('#guardar_subcoto').submit();
    } else {
        $('#guardar_subcoto').show();
        swal({
...
$( "input#nombre_subcoto" ).blur(function() {
...
$( "select#cotos_id" ).change(function() {
    validarSelect($(this));
});
...
function validarInput (campo,regExpr) {
    ...
    ($(campo).attr('name') == 'telefono_2' && $(campo).val() == '')) {
        ...
        return true;
    } else if ($(campo).attr('name') == 'password' && $(campo).val() == '' && $("#formulario_subcoto input#id").val() != '') {
        $(campo).parent().removeClass("has-error");
        return true;
